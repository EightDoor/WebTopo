...
export function checkByPointInRect (frameRect, itemRect) {
  ...
  if (x > frameRect.x && y > frameRect.y && x < (frameRect.x + frameRect.width) && y < (frameRect.y + frameRect.height)) { return true } else { return false }
}
...
export function checkByRectCollisionDetection (frameRect, itemRect) {
  const x1 = frameRect.x + frameRect.width / 2,
    y1 = frameRect.y + frameRect.height / 2,
    w1 = frameRect.width,
    h1 = frameRect.height
  const x2 = itemRect.x + itemRect.width / 2,
    y2 = itemRect.y + itemRect.height / 2,
    w2 = itemRect.width,
    h2 = itemRect.height
  if (Math.abs(x1 - x2) < ((w1 + w2) / 2) && Math.abs(y1 - y2) < ((h1 + h2) / 2)) { return true } else { return false }
}
