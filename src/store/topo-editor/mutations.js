  ...
  switch (command.op) {
    ...
    case 'del':
      ...
      for (let i = 0; i < state.topoData.components.length; i++) {
        const identifier = state.topoData.components[i].identifier
...
export const undo = state => {
  ...
  switch (command.op) {
    case 'add':
      // 逆向循环删除
      for (let i = state.topoData.components.length - 1; i >= 0; i--) {
        if (command.component.identifier === state.topoData.components[i].identifier) {
    ...
    case 'move':
      var dx = command.dx,
        dy = command.dy
      for (const key in state.selectedComponentMap) {
...
export const removeSelectedComponent = (state, component) => {
  ...
  // 如果只有一个组件，则默认选中
  if (state.selectedComponents.length === 1) {
    const _component = state.selectedComponentMap[state.selectedComponents[0]]
